...
//pascal
	...
	typedef struct tagNkMAIDUIRequestInfo
	{
		...
		ULONG				ulDefault;		// default return value ñ one of eNkMAIDUIRequestResult
		_BOOL				fSync;			// TRUE if user must respond before returning
	...
	typedef struct tagNkMAIDImageInfo
	{
		...
		_BOOL			fRemoveObject;	// TRUE if the object should be removed
	...
	typedef struct tagNkMAIDSoundInfo
	{
		...
		_BOOL		fStereo;				// TRUE if stereo, FALSE if mono
	...
	typedef struct tagNkMAIDFileInfo
	{
		...
		_BOOL				fDiskFile;			// TRUE if the file is delivered on disk
		_BOOL				fRemoveObject;		// TRUE if the object should be removed
...
// The module will deliver one of these values to the nResult parameter of the clientís
...
// The module will deliver one of these values to the ulEvent parameter of the clientís event
...
// When the module calls the clientís user interface callback function, the ulType member of
...
// When the module calls the clientís user interface callback function, the ulDefault member
// of the NkMAIDUIRequestInfo structure will be set to one of these values.  The value will
// indicate which button should be highlighted by default.  The clientís user interface callback
...
enum eNkMAIDCommand
{
	...
	kNkMAIDCommand_EnumChildren,	// requests ëaddí events for all child IDs
